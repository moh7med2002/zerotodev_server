...
export class CategoryService {
    ...
    async create(body:createCategoryDto)
    {
        ...
        const newCategory = await this.categoryRepo.create({name:name.toLowerCase()})
    ...
    {
        const categoryByName = await this.categoryRepo.findOne({where:{name:name.toLowerCase()}})
        if(categoryByName)
        {
